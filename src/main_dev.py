...
def run(args):
    ...
    # Single feature type
    # --------------------
    for fea_name in data_dct.keys():
        print(f'Processing feature {fea_name} ...')
        data = data_dct[fea_name]
        # For every split, train model, and dump prediction and scores

        # Get features (x), target (y), and meta
        # fea_list = fea_prfx_dct[fea_name]
        xdata = extract_subset_fea(data, fea_list=fea_prfx_dct[fea_name], fea_sep='.')
        meta = data.drop( columns=xdata.columns )
        ydata = meta[[ args['target_name'] ]]
        del data

        # all_split_files = glob( str(splitdir/'1fold_*_id.csv' )
        all_split_files = sorted(splitdir.glob('1fold_*_id.csv'))
        ...
        for i, split_id in enumerate(unq_split_ids[:n_splits]):
            ...
            trn_single_split( split_id, all_split_files, xdata, ydata, meta,
                    ml_model_def, ml_init_kwargs, ml_fit_kwargs )
    ...
    def trn_single_split( split_id, all_split_files, xdata, ydata, meta,
        ...
        if framework=='lightgbm':
            model, trn_outdir, runtime = trn_lgbm_model(model=model, ml_fit_kwargs=ml_fit_kwargs, xtr=xtr, ytr=ytr, eval_set=eval_set)
        elif framework=='sklearn':
            model, trn_outdir, runtime = trn_sklearn_model(model=model, xtr=xtr, ytr=ytr, eval_set=None)
        elif framework=='keras':
            model, trn_outdir, runtime = trn_keras_model(model=model, xtr=xtr, ytr=ytr, eval_set=eval_set)
    ...
    all_split_files = glob( splitdirstr(Path(args['splitpath'])/'1fold_*_id.csv'))
    unq_split_ids = get_unq_split_ids(all_split_files)
    run_times = []
    ...
    for i, split_id in enumerate(unq_split_ids[:n_splits]):
        ...
        # Train single splits
        trn_single_split( split_id, xdata, ydata, meta, ml_model_def )
